    ...
    def handle(self, *args, **kwargs):
        ...
        for entity in entities:
            ...
            if created:
                ...
                if updated_fields:
                    obj.save()
                    self.stdout.write(self.style.WARNING(f"✏️ Updated {obj.name}"))
                    for f, v in updated_fields.items():
                        self.stdout.write(f"   {f}: '{v['old']}' → '{v['new']}'")
                else:
                    self.stdout.write(self.style.NOTICE(f"ℹ️ No changes: {obj.name}"))

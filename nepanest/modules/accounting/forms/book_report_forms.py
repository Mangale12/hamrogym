...
from nepanest.foundation.organization import Branch, Organization

from ..models import ChartOfAccount, VoucherType
from ..services.books import is_special_book_account


def _ledger_label(account: ChartOfAccount) -> str:
    return f"{account.code or 'AUTO'} - {account.full_path}"


class _BaseScopeReportForm(forms.Form):
...
class GeneralLedgerReportForm(_BaseScopeReportForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["account"].label_from_instance = _ledger_label


class SpecialBookReportForm(_BaseScopeReportForm):
    account = forms.ModelChoiceField(
        queryset=ChartOfAccount.objects.filter(is_ledger=True, is_active=True).select_related("parent").order_by("code", "name"),
        required=False,
    )

    def __init__(self, *args, book_type: str = "", **kwargs):
        self.book_type = book_type
        super().__init__(*args, **kwargs)
        self.fields["account"].required = False
        self.fields["account"].empty_label = "All Matching Ledgers"
        self.fields["account"].label_from_instance = _ledger_label

    def clean(self):
        cleaned_data = super().clean()
        account = cleaned_data.get("account")
        if account and self.book_type and not is_special_book_account(account, self.book_type):
            self.add_error("account", f"Selected ledger does not qualify for the {self.book_type} book.")
        return cleaned_data


class DayBookReportForm(_BaseScopeReportForm):
    voucher_type = forms.ModelChoiceField(
        queryset=VoucherType.objects.filter(is_active=True).order_by("name"),
        required=False,
    )

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["voucher_type"].empty_label = "All Voucher Types"
